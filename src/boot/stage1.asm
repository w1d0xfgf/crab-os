...
db 65			; Количество зарезервиртованных секторов
db 2			; Количество FAT
dw 224			; Количество записей в корневой директории
dw 2880			; Обычное количество секторов
db 0xF0			; Тип хранилища
dw 9			; Секторов на FAT
dw 18			; Секторов на дорожку
dw 2			; Количество головок
dd 0			; LBA начала раздела
dd 0			; Большое количество секторов (если Обычное количество секторов = 0)
...
db 0x29				; Сигнатура
db 'CRAB'			; VolumeID
db 'CrabOS0.1.8'	; Volume Метка
db 'FAT16   '		; Файловая система
...
start:
	...
	mov bx, 0x8000			; Адрес в памяти
	xor ax, ax				; AX = 0
	mov es, ax				; ES = 0
	mov ah, 0x02			; AH = 0x02 -- BIOS чтение секторов в память
	mov al, 64				; Число секторов
	mov ch, 0				; Cylinder = 0
	mov cl, 2				; Читать начиная с сектора 2 (бутлоадер в секторе 1)
	mov dh, 0				; Head = 0
